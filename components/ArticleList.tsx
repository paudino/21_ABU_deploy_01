...
interface ArticleListProps {
  ...
  onCloseFavorites?: () => void; // Nuova prop per chiudere la vista preferiti
}
...
export const ArticleList: React.FC<ArticleListProps> = ({
  ...
  // Effetto per generare immagini AI in background per gli articoli che non ne hanno una
  useEffect(() => {
  ...
  // Mostra gli scheletri solo se stiamo caricando E la lista è vuota.
  const showSkeletons = loading && articles.length === 0;

  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        
      {/* Header Sezione con effetto vetro */}
      ...
      {/* Stato Caricamento (Skeletons) - Solo se lista vuota */}
      {showSkeletons ? (
          ...
          {articles.length === 0 ? (
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-12 transition-opacity duration-300 ${loading ? 'opacity-70 grayscale-[30%]' : 'opacity-100'}`}>
              {articles.map((article, idx) => {
                // Se siamo nei preferiti, il pulsante cuore è sempre pieno (perché è un preferito).
                // Altrimenti controlliamo il set.
                    ...
                    <div className="p-6 flex-1 flex flex-col rounded-b-2xl">
                      ...
                      <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-sm text-slate-400 font-sans">
                          ...
                          <div className="flex items-center gap-4">
                               ...
                               {/* Icone Like/Dislike con Contatori */}
